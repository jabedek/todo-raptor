...
import { MdPlaylistRemove, MdReadMore, MdInventory } from "react-icons/md";
...
type Props = {
  ...
  deleteFn: CallbackFn<Promise<any>>;
};

const AVATARS_LIMIT = 5;
const TAGS_CHARS_LIMIT = 37;

const ProjectTableItem: React.FC<Props> = ({ project, user, deleteFn }) => {
        ...
        <div
          ...
          <div className="flex w-full justify-start min-h-[28px] h-[24px] my-[4px] ml-[3px] px-[10px]">
            {membersLimited.map((m, i) => (
              <div
                key={i}
                className="team-member relative font-app_mono ml-[-5px] text-[10px] h-[24px] w-[24px] rounded-full bg-green-300 app_flex_center border-2 border-white border-solid">
          ...
          <div className="flex w-full justify-between items-center px-[10px]">
            ...
            <div className="flex justify-between gap-2 min-w-[60px] ">
              ...
              <div
                className="action-wrapper h-[26px] w-[26px] app_flex_center rounded-[3px] bg-white hover:bg-slate-100 group transition-all transition-200 cursor-pointer "
                onClick={popupDelete}>
                <MdPlaylistRemove className="h-[16px] w-[16px] font-[300] text-gray-500 group-hover:text-red-700" />
              </div>
              <div
                className="action-wrapper h-[26px] w-[26px] app_flex_center rounded-[3px] bg-white hover:bg-slate-100 group transition-all transition-200 cursor-pointer "
                onClick={() => navigate(`/projects/${project.id}`)}>
                <MdReadMore className="h-[16px] w-[16px] font-[300] text-gray-500 group-hover:text-blue-700" />
              </div>
            </div>
          </div>
        </div>
