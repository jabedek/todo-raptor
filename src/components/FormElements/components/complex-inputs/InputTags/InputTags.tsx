...
import { FormClearX, InputTag, InputTagsTypes } from "@@components/FormElements";
...
const InputTags: React.FC<InputTagsTypes.InputTagsProps> = (props) => {
  ...
  const handleBlur = () => {
    const newValues: InputTagsTypes.TagItem[] = [...values].filter((v: InputTagsTypes.TagItem) => v.value && v.id);
  ...
  const onDelete = (_: React.MouseEvent, index: string) => {
    updateValues([...values].filter((v) => v.id !== index));
  };
