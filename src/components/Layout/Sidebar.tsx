...
const Sidebar: React.FC<Props> = ({ isOpened, setState }) => (
  <>
    <div
      ...
      z-[49] fixed sidebar-overlay app_flex_center  w-[60px] h-[50px] bg-gray-300 bg-opacity-70 text-sm font-semibold font-app_primary rounded-r-full cursor-pointer`}
        onClick={() => {
          setState(!isOpened);
        }}>
    ...
    <div
      className={`fixed z-30 app_header_top w-full h-full transition-all duration-200 ${
        isOpened ? "bg-[rgba(0,0,0,0.15)]" : "bg-none"
      } `}></div>
  </>
);
